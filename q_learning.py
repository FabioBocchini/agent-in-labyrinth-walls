...
class QLearning(object):
    ...
    def execute(self, step_by_step=False):
        ...
        while command != "n":
            if step_by_step:
                command = input("ExecuteNext?(y/n/uP/dOWN/lEFT/rIGTH):")
            else:
                time.sleep(0.5)

            _, obs = self.env.next_observation()

            if command == "u":
                action = labyrinth_env.actions["U"]
            elif command == "d":
                action = labyrinth_env.actions["D"]
            elif command == "l":
                action = labyrinth_env.actions["L"]
            elif command == "r":
                action = labyrinth_env.actions["R"]
            else:  # y
                action = self.max_action(q, obs, self.env.possible_actions)
